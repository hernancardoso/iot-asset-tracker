...
#define LIS2DH_RANGE_MAX_M_S2 19.6133
#elif defined(CONFIG_LIS2DH_ACCEL_RANGE_4G)
#define LIS2DH_RANGE_MAX_M_S2 39.2266
#elif defined(CONFIG_LIS2DH_ACCEL_RANGE_8G)
#define LIS2DH_RANGE_MAX_M_S2 78.4532
#elif defined(CONFIG_LIS2DH_ACCEL_RANGE_16G)
#define LIS2DH_RANGE_MAX_M_S2 156.9064
#endif

//#define ADXL362_THRESHOLD_RESOLUTION_DECIMAL_MAX 2048
#define LIS2DH_THRESHOLD_RESOLUTION_DECIMAL_MAX 512
...
double threshold = LIS2DH_RANGE_MAX_M_S2;
...
int ext_sensors_mov_thres_set(double threshold_new)
{
	...
	//double range_max_m_s2 = ADXL362_RANGE_MAX_M_S2;
        double range_max_m_s2 = LIS2DH_RANGE_MAX_M_S2;
	...
	/* Convert threshold value into 9-bit decimal value relative
	 * to the configured measuring range of the accelerometer.
	 */
	threshold_new = (threshold_new *
		(LIS2DH_THRESHOLD_RESOLUTION_DECIMAL_MAX / range_max_m_s2));
 ...
 //       if (input_value >= ADXL362_THRESHOLD_RESOLUTION_DECIMAL_MAX) {
	...
	if (input_value >= LIS2DH_THRESHOLD_RESOLUTION_DECIMAL_MAX) {
		input_value = LIS2DH_THRESHOLD_RESOLUTION_DECIMAL_MAX - 1;
	} else if (input_value < 0) {
		input_value = 0;
	}
	...
	if (err) {
		...
		LOG_ERR("Device: %s, error: %d,data: %d ",
			accel_sensor.dev->name, err, data.val1 );
		evt.type = EXT_SENSOR_EVT_ACCELEROMETER_ERROR;
		evt_handler(&evt);
		return err;
	}
